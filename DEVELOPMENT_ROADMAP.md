...
**当前阶段：[阶段一] 已完成，[阶段二] 进行中。**
基础设施（底层数据库、缓存、对象存储、控制面及计算引擎）已全部通过 Docker Compose 成功点亮并健康运行。当前核心研发精力集中在 **数据摄入管道（Data Ingestion Pipeline）** 的打通，以及端到端（文件 -> Worker -> Doris）的连通性验证。
...
- [ ] **Cube.js Doris 方言适配**: 确保 Cube.js 能够正确将逻辑查询下推翻译为 Doris 优化的 SQL。
...
- [ ] **两级缓存体系建设**: 调优 Redis 结果缓存和 CubeStore 预聚合引擎，确保 500+ 并发下的大盘秒级响应。
...
1. **Cube.js 模型联调**: 手动在 Doris 中建表并写入少量测试数据，然后在 Cube.js 中编写对应的静态 Schema，验证聚合查询和接口响应速度。
2. **元数据闭环**: 联调 Directus 创建数据源 -> 触发 Webhook -> Data Worker 建表 -> Cube.js 热加载 Schema 的完整链路。