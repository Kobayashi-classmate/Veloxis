...
核心 D/C/D 架构已贯通。从 Directus 用户操作 -> 触发 Webhook -> Data Worker 解析与 Doris 高并发入库 -> 动态生成 Cube.js Schema -> CubeStore 预聚合与租户级权限管控，整个端到端生命周期已经得到程序化验证。前端工作台脚手架已搭建。
...
2. **数据源管理视图**: 开发前端 `Data Sources` 页面，实现直接在 UI 中上传文件并创建 Dataset，连通后端工作流。