...
import {
  Typography, Row, Col, Card, Button, Modal, Input,
  Tag, Dropdown, Empty, message, Skeleton,
} from 'antd'
import {
  PlusOutlined, BarChartOutlined, MoreOutlined,
  EditOutlined, DeleteOutlined, EnterOutlined, AppstoreOutlined,
} from '@ant-design/icons'
import dayjs from 'dayjs'
import relativeTime from 'dayjs/plugin/relativeTime'
import 'dayjs/locale/zh-cn'
import {
  getWorkbooks, createWorkbook, updateWorkbook, deleteWorkbook,
...
const Workbooks = () => {
  ...
  const handleCreate = async () => {
    ...
    if (!workbookSlug) { message.warning('请输入工作台标识符'); return }
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <Title level={4} style={{ margin: 0 }}>可视化工作台</Title>
      ...
      <Modal
        ...
        onOk={handleCreate}
        onCancel={() => { setCreateModalOpen(false); setNewName(''); setNewSlug(''); setSlugManuallyEdited(false); setNewDesc('') }}
        okText="创建并进入"
        cancelText="取消"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 16 }}>
          ...
          <div>
            ...
            <Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 6 }}>
              用于 URL 路径，如
              <Text code style={{ fontSize: 11, marginLeft: 4 }}>/workbooks/{`wb-${newSlug || 'sales-q1'}`}</Text>
            </Text>
          </div>
          <div>
            <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 6 }}>
