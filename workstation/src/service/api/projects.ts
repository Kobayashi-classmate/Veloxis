...
function normalize(
  raw: DirectusProject,
  memberRecord: ProjectMember | undefined,
  starred: boolean,
): Project {
  return {
    id:          raw.id,
...
export async function getProjects(): Promise<Project[]> {
  ...
  const PROJECT_FIELDS = [
    'id', 'name', 'description', 'status', 'visibility', 'tenant', 'color',
    'tags', 'storage_label', 'cube_health', 'doris_latency', 'last_active',
    'members_count', 'datasets_count', 'workbooks_count', 'recipes_count',
  ].join(',')
  ...
  const starredSet = new Set(starredList.map((s: any) => {
    return typeof s.project_id === 'string' ? s.project_id : s.project_id?.id
  }))

  return rawProjects.map((raw) =>
    normalize(raw, memberMap.get(raw.id), starredSet.has(raw.id))
  )
}
 ...
 */
export async function toggleStarProject(
  projectId: string,
  starred: boolean,
): Promise<void> {
  const userId = authService.getState().user?.id
 ...
 */
export async function createProject(data: { name: string, description?: string, tenant?: string, visibility?: ProjectVisibility, color?: string }): Promise<Project> {
...
export async function getProjectMembers(projectId: string): Promise<ProjectMember[]> {
    const res = await request.get('/items/project_members', {
        'filter[project_id][_eq]': projectId,
        fields: 'id,project_id,role,directus_users_id.id,directus_users_id.first_name,directus_users_id.last_name,directus_users_id.email',
        limit: -1
    });
    return (res as any)?.data ?? res ?? [];
}
